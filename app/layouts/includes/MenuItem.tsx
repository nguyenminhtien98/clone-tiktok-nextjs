...
export default function MenuItem({ iconString, colorString, sizeString }: MenuItemTypes) {
    ...
    const icons = () => {
        if (iconString == 'Dành cho bạn') return <AiOutlineHome size={sizeString} color={colorString} />
        if (iconString == 'Đang Follow') return <RiGroupLine size={sizeString} color={colorString} />
