...
import { fetchProfilesByName } from "../../hooks/useSearchProfilesByName";
...
export default function TopNav() {
  ...
  const SearchProfileItem = ({ profile }: { profile: Profile }) => {
    const profileImageUrl = useCreateBucketUrl(profile?.image);
    ...
    return (
      ...
      >
        <div className="flex items-center">
          <img className="rounded-md" width="40" src={profileImageUrl} />
  ...
  const handleSearchName = debounce(
    async (event: { target: { value: string } }) => {
      ...
      try {
        const result = await fetchProfilesByName(event.target.value);
            ...
            {searchProfiles.length > 0 ? (
              <div className="absolute bg-white max-w-[910px] h-auto w-full z-20 left-0 top-12 border p-1">
                {searchProfiles.map((profile, index) => {
                  return <SearchProfileItem key={index} profile={profile} />;
                })}
              </div>
            ) : null}
